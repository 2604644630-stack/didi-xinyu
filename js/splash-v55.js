...
/* ============================================================
   启动页：照抄 Mineradio 入场动画（WebGL 着色器 + 2D 粒子）
   ============================================================ */
(function () {
  ...
  function initSplashWebgl(canvas) {
    ...
    var fragmentSource = [
      ...
      '  vec3 ch1 = vec3(1.00, 0.52, 0.36);',
      '  vec3 ch2 = vec3(1.00, 0.80, 0.38);',
      '  vec3 ch3 = vec3(1.00, 0.92, 0.66);',
      ...
      '  col += vec3(0.034, 0.017, 0.010) * (1.0 - vignette);',
    ...
    try {
      ...
      if (ctx.state === 'suspended' && ctx.resume) {
        ctx.resume().then(function () {
          if (!splashSoundPlayed) playSplashIntroSound();
        }).catch(function () { });
  ...
  function initSplashCanvas() {
    ...
    if (!reduceSplashMotion && initSplashWebgl(splashCanvas)) {
      splashCtx = null;
    } else {
  ...
  function drawSplash() {
    ...
    base.addColorStop(0, 'rgba(30,18,10,0.72)');
    base.addColorStop(0.45, 'rgba(26,16,10,0.76)');
    base.addColorStop(1, 'rgba(16,9,5,0.86)');
    ...
    if (lineT > 0 && exitFade > 0) {
      ...
      slitGrad.addColorStop(0, 'rgba(255,150,110,0)');
      slitGrad.addColorStop(0.18, 'rgba(255,150,110,' + (0.18 * exitFade).toFixed(3) + ')');
      slitGrad.addColorStop(0.50, 'rgba(255,255,255,' + coreAlpha.toFixed(3) + ')');
      slitGrad.addColorStop(0.68, 'rgba(255,206,130,' + (0.38 * exitFade).toFixed(3) + ')');
      slitGrad.addColorStop(0.84, 'rgba(255,180,140,' + (0.20 * exitFade).toFixed(3) + ')');
      slitGrad.addColorStop(1, 'rgba(255,180,140,0)');
      splashCtx.shadowColor = 'rgba(255,206,130,' + (0.48 * exitFade).toFixed(3) + ')';
      ...
      if (ignition > 0.018) {
        ...
        ig.addColorStop(0, 'rgba(255,180,140,0)');
        ig.addColorStop(0.46, 'rgba(255,180,140,' + (0.07 * ignition).toFixed(3) + ')');
        ig.addColorStop(0.50, 'rgba(255,255,255,' + (0.16 * ignition).toFixed(3) + ')');
        ig.addColorStop(0.54, 'rgba(255,150,110,' + (0.08 * ignition).toFixed(3) + ')');
        ig.addColorStop(1, 'rgba(255,206,130,0)');
      ...
      if (waveAlpha > 0) {
        ...
        splashCtx.strokeStyle = 'rgba(255,206,130,' + (0.22 * waveAlpha).toFixed(3) + ')';
      ...
      if (flash > 0.015) {
        ...
        fg.addColorStop(0, 'rgba(255,150,110,0)');
        fg.addColorStop(0.48, 'rgba(255,255,255,' + (0.20 * flash).toFixed(3) + ')');
        fg.addColorStop(0.52, 'rgba(255,206,130,' + (0.24 * flash).toFixed(3) + ')');
        fg.addColorStop(1, 'rgba(255,180,140,0)');
  ...
  function markSplashReadyToEnter() {
    ...
    s.setAttribute('aria-label', '点击进入 迪迪的心屿');
  }
  ...
  function dismissSplash() {
    ...
    setTimeout(stopSplashIntroSound, 240);
    ...
    setTimeout(function () {
      ...
      if (s.parentNode) s.style.display = 'none';
    }, 620);
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    initSplashCanvas();
    s.dataset.renderer = splashGl ? 'webgl' : '2d';

    function requestSplashEnter() {
      playSplashIntroSound();
      if (splashReadyToEnter) {
        dismissSplash();
      } else {
        // 任意时刻点击都立即进入，无需等待就绪计时器
